...
class MarkersPanel(wx.Panel, ColumnSorterMixin):
    def __init__(self, parent, nav_hub):
        ...
        # ---  ДОДАЄМО СТОВПЕЦЬ ЧАСУ ---
        # Визначаємо індекс для колонки часу (беремо кількість існуючих колонок)
        self.TIME_COLUMN_IDX = marker_list_ctrl.GetColumnCount()
        
        marker_list_ctrl.InsertColumn(self.TIME_COLUMN_IDX, "Time")
        marker_list_ctrl.SetColumnWidth(self.TIME_COLUMN_IDX, 130) # Ширина 130 пікселів для часу
        # ---------------------------------------
    ...
    def _AddMarker(self, marker, render, focus):
       ...
       # --- ПЕРЕДАЄМО ПОВНУ ДАТУ Й ЧАС У ТАБЛИЦЮ ---
        if hasattr(marker, 'timestamp') and marker.timestamp:   
            time_str = str(marker.timestamp).replace('T', ' ')
        else:
            time_str = ""
            
        list_entry.insert(self.TIME_COLUMN_IDX, time_str)
        # -----------------------------------------------------
